...
void centralization(pcl::PointCloud<pcl::PointXYZ>::Ptr input)
{
  //中心化
  Eigen::Vector4f centroid;  //质心 
  pcl::compute3DCentroid(*input,centroid); //估计质心的坐标
  Eigen::Matrix4f centroid_matrix = Eigen::Matrix4f::Identity();
  centroid_matrix.block<4,1>(0,3) = -centroid;
  pcl::transformPointCloud (*input, *input, centroid_matrix);

//   std::cout << centroid << std::endl;
//   std::cout << centroid_matrix << std::endl;
}
...
main (int argc, char** argv)
{
  ...
  if (pcl::io::loadPCDFile<pcl::PointXYZ> ("../2d_target.pcd", *target_cloud) == -1)
  {
  ...
  if (pcl::io::loadPCDFile<pcl::PointXYZ> ("../2d_source.pcd", *input_cloud) == -1)
  {
  ...
  //中心化
  centralization(target_cloud);
  centralization(input_cloud);
  ...
  //add x,y,theta bias
	...
	0,0,1,0,
	0,0,0,1;
    float angle_a = 30.0/180.0*3.14;
    std::cout << angle_a << std::endl;

    std::cout << cos(angle_a) << std::endl;
    T(0,0) = std::cos(angle_a);
    T(0,1) = std::sin(angle_a);
    T(1,0) = -std::sin(angle_a);
    T(1,1) = std::cos(angle_a);
    std::cout << "T:" << std::endl;
    std::cout << T << std::endl;
  
    pcl::transformPointCloud(*input_cloud, *input_cloud, T);
  
    viewPair(target_cloud,input_cloud,target_cloud,input_cloud);
...
//而是使用包含在每个体素单元格中的点的统计数据
  ...
  ndt.setStepSize (7.0);              // 为more-thuente线搜索设置最大步长
  ndt.setResolution (20.0);            // 设置NDT网格网格结构的分辨率（voxelgridcovariance）
